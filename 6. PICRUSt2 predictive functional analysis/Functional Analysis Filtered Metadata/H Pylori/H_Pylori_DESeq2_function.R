...
# Update the subject part to your own metadata column of interest
  DESeq2_colnames[DESeq2_colnames == col_of_interest] <- "Stage_Pylori"
  colnames(DESeq2_metadata) <- DESeq2_colnames
  DESeq2_metadata = as.data.frame(DESeq2_metadata)
  DESeq2_metadata[,"Stage_Pylori"] <- as.factor(DESeq2_metadata[,"Stage_Pylori"])
  
  # Generate combinations of groups for comparison
  DESeq2_combinations <- utils::combn(unique(DESeq2_metadata[, "Stage_Pylori"]), 2)
  DESeq2_results <- list()
  ...
  for (i in seq_len(ncol(DESeq2_combinations))) {
    ...
    DESeq2_sub_group <- DESeq2_metadata$Stage_Pylori %in% j
