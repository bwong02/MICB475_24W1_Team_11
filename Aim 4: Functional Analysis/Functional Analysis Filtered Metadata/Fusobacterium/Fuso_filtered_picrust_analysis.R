...
#make plot
Ordered_Healthy_FilteredFusoLowVSHigh_DESeq2 <- ggplot(data = healthyFusoLowvsHigh_sig_res, aes(y = reorder(description, -log2FoldChange), x= log2FoldChange, fill = pvalue))+
...
Ordered_CG_FilteredFusoLowVSHigh_DESeq2 <- ggplot(data = cgFusoLowvsHigh_sig_res, aes(y = reorder(description, -log2FoldChange), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity") +
  theme_test()+
  theme(plot.margin = margin(t = 10, r = 10, b = 30, l = 10), 
        axis.text.y = element_text(size = 12), 
...
Ordered_IM_FilteredFusoLowVSHigh_DESeq2 <- ggplot(data = imFusoLowvsHigh_sig_res, aes(y = reorder(description, -log2FoldChange), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity") +
  theme_test()+
  theme(plot.margin = margin(t = 10, r = 10, b = 30, l = 10), 
        axis.text.y = element_text(size = 12), 
...
Ordered_INeo_FilteredFusoLowVSHigh_DESeq2 <- ggplot(data = ineoFusoLowvsHigh_sig_res, aes(y = reorder(description, -log2FoldChange), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity") +
  theme_test()+
  theme(plot.margin = margin(t = 10, r = 10, b = 30, l = 10), 
        axis.text.y = element_text(size = 12), 
...
Stacked_Fuso_UpVSDown_Counts_Summary <- ggplot(summary_long, aes(x = Group, y = Count, fill = Log2FC_Type)) +
  geom_bar(stat = "identity") +
  ...
  scale_fill_manual(values = c("Positive_Log2FC" = "lightblue", "Negative_Log2FC" = "salmon"), 
                    labels = c("Uprepresented in Fusobacterium High", "Uprepresented in Fusobacterium Low")) +  # Custom colors
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
          plot.title = element_text(hjust = 0.5))

OneColour_Stacked_Fuso_UpVSDown_Counts_Summary <- ggplot(summary_long, aes(x = Group, y = Count, fill = Group)) +
  geom_bar(stat = "identity") +
  labs(title = expression(atop("Comparison of Significantly Changed Pathways between ", italic(Fusobacterium )~" Abundance Across Disease Stages")), 
...
sideBySide_Fuso_UpVSDown_Counts_Summary <- ggplot (summary_long, aes (x = Group, y = Count, fill = Log2FC_Type)) + 
