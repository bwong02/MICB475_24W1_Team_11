...
DEseq2_function = function(abundance_table,metadata,col_of_interest){
  ...
  DESeq2_abundance_mat <- abundance_table %>% column_to_rownames("#OTU ID")
  
  
  DESeq2_colnames <- colnames(DESeq2_metadata)
# Update the subject part to your own metadata column of interest
  DESeq2_colnames[DESeq2_colnames == col_of_interest] <- "Stage_FusoAbundance"
  colnames(DESeq2_metadata) <- DESeq2_colnames
  DESeq2_metadata = as.data.frame(DESeq2_metadata)
  DESeq2_metadata[,"Stage_FusoAbundance"] <- as.factor(DESeq2_metadata[,"Stage_FusoAbundance"])
  
  # Generate combinations of groups for comparison
  DESeq2_combinations <- utils::combn(unique(DESeq2_metadata[, "Stage_FusoAbundance"]), 2)
  DESeq2_results <- list()
  ...
  for (i in seq_len(ncol(DESeq2_combinations))) {
    ...
    DESeq2_sub_group <- DESeq2_metadata$Stage_FusoAbundance %in% j
