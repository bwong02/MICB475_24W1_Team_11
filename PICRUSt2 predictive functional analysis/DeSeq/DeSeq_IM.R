...
sample_data(ps_plus1)$Group <- factor(sample_data(ps_plus1)$Group, levels = c("Healthy control (HC)", "Intestinal metaplasia (IM）"))
...
ps_plus1 <- subset_samples(ps_plus1, Group %in% c("Healthy control (HC)", "Intestinal metaplasia (IM）"))
...
plot <- ggplot(top_50_ASVs) +
  ...
  labs(x = "Genus", y = "Log2 Fold Change", title = "Top 50 Significant ASVs: Intestinal Metaplasia vs. Healthy Control")


# Save the bar plot as a PNG file in the project folder
ggsave("ps_deseq_plot_top50_IM.png", plot = plot, height = 6, width = 10)
