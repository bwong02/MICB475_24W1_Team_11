...
metadata$fuso_detected <- "Undetected"
...
if (length(fuso_otu) > 0) {
  ...
  detected_samples_high <- fuso_percentage > 10

  # Set "Yes" for samples with detected Fusobacterium (i.e., percentage > 1%)
  metadata$fuso_detected[detected_samples] <- "Low"
  metadata$fuso_detected[detected_samples_high] <- "High"
}
